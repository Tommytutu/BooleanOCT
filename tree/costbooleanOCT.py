# -*- coding: utf-8 -*-
"""
Created on Sat Nov 25 22:16:35 2023
...
class costbooleanoptimalDecisionTreeClassifier:
    """
    optimal classification tree
    """
    def __init__(self, max_depth=3, min_samples_split=1, alpha=0.01,N=5, warmstart=True, timelimit=600, output=True,c0=1,c1=1):
    ...
    def _buildMIP(self, x, y):
        ...
        L = m.addVars(self.l_index, self.labels, vtype=GRB.CONTINUOUS, name='L') # leaf node misclassified
        ...
        FN = gp.quicksum(L[t,0] for t in self.l_index)
        FP = gp.quicksum(L[t,1] for t in self.l_index)
        
        if sum(y)> self.n - sum(y):
            basecost=self.c0 *(self.n - sum(y))
        else:
            basecost=self.c1 *sum(y)
            

        # objective function
        error=(self.c0 * FP + self.c1 * FN)/basecost
        ...
        m.addConstrs(L[t,k] >= z.sum('*', t) - gp.quicksum((y[i] == k) * z[i,t] for i in range(self.n)) -self.n * (1 - c[k,t]) for k in self.labels for t in self.l_index)

        m.addConstrs(L[t,k] <= z.sum('*', t) - gp.quicksum((y[i] == k) * z[i,t] for i in range(self.n))   for k in self.labels for t in self.l_index)

        m.addConstrs(L[t,k] <= self.n*c[k,t]   for k in self.labels for t in self.l_index)
